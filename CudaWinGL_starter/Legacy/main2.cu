...
int nx = WIDTH;
int ny = HEIGHT;
int ns = 8; // ray shoot. more = more real + more lag
int tx = 24; // thread block x
int ty = 24; // thread block y
...
// int skybox_width, skybox_height, skybox_channels;
// unsigned char* skyboxImage
__device__ vec3 color(const ray& r, const spheres  world, curandState* local_rand_state, 
                        int skybox_width, int skybox_height, unsigned char* skyboxImage) {
    ray cur_ray = r;
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (world.hitall(cur_ray, 0.001f, FLT_MAX, rec)) {
            ...
            // spherical coords
            float theta = atan2f(unit_direction.z(), unit_direction.x()); 
            float phi = acosf(unit_direction.y()); 
            ...
            int px = int(u * skybox_width);
            int py = int(v * skybox_height);

            // clamp

            int idx = (py * skybox_width + px) * 3;

            return vec3(skyboxImage[idx] / 255.0f, skyboxImage[idx + 1] / 255.0f, skyboxImage[idx + 2] / 255.0f);
        }
    }
...
__global__ void render(unsigned int* fb, int max_x, int max_y, int ns, __grid_constant__ const camera cam,
    ...
    for (int s = 0; s < ns; s++) {
        ...
        col += color(r, world, &local_rand_state, skybox_width, skybox_height, skyboxImage);
    }
...
#pragma region "gl setup"
    ...
    skyboxImage = stbi_load("fireplace.jpg", &skybox_width, &skybox_height, &skybox_channels, 0);
    if (skyboxImage == NULL) {
...
#pragma endregion "gl setup"
    ...
    while (!glfwWindowShouldClose(window))//(Msg.message != WM_QUIT)
    {
        ...
        render << <blocks, threads >> > (cuda_dest_resource, nx, ny, ns, d_camera, a_world, d_rand_state, skybox_width, skybox_height, skyboxImage);
        checkCudaErrors(cudaGetLastError());
        //checkCudaErrors(cudaDeviceSynchronize());
        // 
