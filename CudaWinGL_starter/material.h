...
__device__ bool scatter_metal(const ray& r_in, const hit_record& rec, vec3& attenuation, ray& scattered, curandState* local_rand_state) {
    ...
    // scatter only into a hemisphere around reflection direction
    vec3 fuzz_dir = rec.fuzz * random_in_unit_sphere(local_rand_state);
    vec3 scatter_dir = reflected + fuzz_dir;

    scattered = ray(rec.p, scatter_dir);
    attenuation = rec.albedo;

    // ensure the ray goes outward
